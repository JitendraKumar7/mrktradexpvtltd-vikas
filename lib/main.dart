...
class _SplashScreenState extends State<SplashScreen> {
  ...
  Widget _infoTile(String title, String subtitle) {
    return Padding(
      ...
      child: ListTile(
        title: Text(title),
        subtitle: Text(subtitle ?? 'Not set',style: TextStyle(color: Colors.white,fontSize: 20,fontWeight: FontWeight.bold),),
      ),
    );
  }
  ...
  void openDashboard() async {
    ...
    if (response['status'] == '200') {
      ...
      print('aaaaaaaaaaaaaaaaaaa$expireresult');
      if(expireresult=="yes"){
setState(() {
  _showMyDialog();
});}else{
  ...
  Future<void> _showMyDialog() async {
    return showDialog<void>(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text('YOUR CARD EXPIRED'),
          content: SingleChildScrollView(
            child: ListBody(
              children: <Widget>[
                Text('Your 1 Year Plan for this appication is expired  '),

                Text('Would you like to UPGRADE/RENEW your BUSINESS CARD ?Please Contact to KONNECT MY BUSINESS  team on this number :'),
InkWell(child: Text("8375938947",style: TextStyle(color: Colors.blue),),onTap: (){setState(() {
  _makePhoneCall('tel:+918375938947');
});
